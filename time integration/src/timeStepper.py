...
#-------------------------------------------------------------------------------------------------#
    def setAdamsBashforth(self,args):
        ...
        self.time = args.tstart
        self.step = 0
        self.cf1=self.coeff[1]
        self.cf2=self.coeff[2]
        self.cf3=self.coeff[3]
...
#-------------------------------------------------------------------------------------------------#
    def runAdamsBashforth(self, Qe, Qb):
        self.report(Qe)
        #-----------------------------------------------------#
        #Initialize time-step loop
        for step in range(self.Nsteps):
            if step==0:
                #Assign Q at t=0
                prevQe = Qe-0
                #Find Q according to First Order Adams Bashfort @time = t+dt where t=0
                Qe += self.dt*self.solver.rhsQ(Qe, Qb)
            else:
                #Apply Second Order Adams-Bashfort without updating Qe
                postQe = ((self.dt)*(self.cf2[0]*self.solver.rhsQ(Qe, Qb)    +   self.cf2[1]*self.solver.rhsQ(prevQe, Qb)))
                #Assign Q @time = t to Q @time = t-dt
                prevQe = Qe-0
                #Update Q for time=t+dt
                Qe += postQe
            #Update global step
            self.step = step
            #produce an output file for postprocessing
...
#-------------------------------------------------------------------------------------------------#
    def runBDF(self, Qe, Qb):
        ...
        #-----------------------------------------------------#
        for step in range(self.Nsteps):
            #Apply first order BDF at step=0
            if (step==0):
                #Store initial Qe for the first step
                prevQe=Qe-0
                Qe[:] = self.solver.solve(self.solver.args, Ap, rhs);
            #Apply Second Order BDF at step>0
            else:
                #Compute b' using BDF coefficients for second order.
                rhs = self.dt*b  + self.cf2[0]*np.multiply(Qe, vol) + self.cf2[1]*np.multiply(prevQe,  vol)
                #Store Qe value at t=t-dt
                prevQe=Qe-0
                #Solve Qe for t=t+dt
                Qe[:] = self.solver.solve(self.solver.args, Ap, rhs);
                #Update global step
